...
const CreatePoint = () => {
  ...
  useEffect(() => {
    axios.get<IBGUfResponse[]>('https://servicodados.ibge.gov.br/api/v1/localidades/estados').then(response => {
      const ufInitials = response.data.map(e => e.sigla);
      setUfs(ufInitials);
    });
  }, []);
  ...
  useEffect(() => {
    ...
    axios.get<IBGCityResponse[]>(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${selectedUf}/municipios`).then(response => {
